...
import { TradeOfferStatus } from "./Offer"
...
export function TradeOffers() {
  ...
  const { data: trades, isLoading: tradesLoading } = useContractRead({
    address: ADDRESSES[chainId],
    abi: PonziRepABI,
    functionName: "getTrades",
  })

  const { data, isLoading: dataLoading } = useContractReads({
    contracts: trades?.map((trade) => ({
      abi: PonziRepABI,
  ...
  if (tradesLoading || dataLoading) {
    return <div className="text-center">Trades loading…</div>
  }
  ...
  return (
    <>
      ...
      <div className="divide-y">
        {data?.length ? (
          data?.map(({ result }, i) => {
            const [uReceive, iReceive, status] = result as [
              bigint,
              bigint,
              TradeOfferStatus
            ]
            return (
              <div className="relative  py-4">
                <Link
                  href={`${trades![i].offerer}/${trades![i].nonce}`}
                  className="flex w-full justify-around text-center"
                >
                  <div className="flex-1">
                    <div>i receive:</div>
                    <div className="text-xl font-semibold">
                      {formatEther(iReceive)} BOB
                    </div>
                  </div>
                  ...
                  {status === TradeOfferStatus.Cancelled && (
                    <>
                      <span className="text-md absolute left-1/2 top-1/2 z-20 -translate-x-1/2 -translate-y-1/2 rotate-[-30deg] rounded border-4 border-red-600 bg-black/50 p-1 font-bold uppercase text-red-600">
                        Cancelled
                      </span>
                    </>
                  )}
                  {status === TradeOfferStatus.Finalised && (
                    <>
                      <span className="text-md absolute left-1/2 top-1/2 z-20 -translate-x-1/2 -translate-y-1/2 rotate-[-30deg] rounded border-4 border-green-600 bg-black/50 p-1 font-bold uppercase text-green-600">
                        Completed!
                      </span>
                    </>
                  )}
